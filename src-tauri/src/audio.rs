...
use crate::vst_host::VstHost; // chain element type stored in vst_chain
...
pub struct AudioEngine {
    ...
    /// Parked (unloaded-but-kept-alive) plugin instances, reused by path on reload.
    /// Plugin Alliance plugins (Lindell 80) keep a process-global graphics engine bound
    /// to the thread that loaded them; the engine survives FreeLibrary. Fully dropping a
    /// host (killing its worker thread + FreeLibrary) orphans that engine, so reloading
    /// the same plugin deadlocks in attached(). Instead of dropping on unload we park the
    /// host here (worker thread + DLL stay alive) and revive it on the next load of the
    /// same path. Parked hosts drop normally on app exit.
    pub vst_parked: Arc<Mutex<Vec<VstHost>>>,
    app: AppHandle,
}
