...
// Fire one re-render the first time #track-list gets a real height.
// The IDB load callback often runs before flex layout settles, so
// clientHeight is 0 when the virtual scroller first computes row ranges.
if (typeof ResizeObserver !== 'undefined') {
  const _tlRO = new ResizeObserver(entries => {
    const h = entries[0]?.contentRect?.height ?? trackList.clientHeight;
    if (h > 0) {
      _tlRO.disconnect();
      if (libraryLoaded) renderCurrentTab();
    }
  });
  _tlRO.observe(trackList);
}
